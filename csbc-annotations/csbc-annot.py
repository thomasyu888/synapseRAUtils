...
##       and unit in days if its a 2 digit # it would only grab the first # 
#RNA_Seq = RNA_Seq.T.drop_duplicates().T
#ATAC_Seq = ATAC_Seq.T.drop_duplicates().T
...
RNA_Seq = pandas.read_csv("Results/RNA_Seq(2).csv")
ATAC_Seq = pandas.read_csv("Results/ATAC_Seq(2).csv")

RNA_Seq.set_index('id', inplace=True, drop=False)
ATAC_Seq.set_index('id', inplace=True, drop=False)

RNA_Seq_dict = RNA_Seq.T.to_dict()
ATAC_Seq_dict = ATAC_Seq.T.to_dict()

for i in RNA_Seq_dict.keys():
	synEntity = syn.get(i,downloadFile = False)
	annotDicts = RNA_Seq_dict.get(i)
	synEntity.annotations = annotDicts
	synEntity = syn.store(synEntity)
	print(synEntity)

for i in ATAC_Seq_dict.keys():
	synEntity = syn.get(i,downloadFile = False)
	annotDicts = ATAC_Seq_dict.get(i)
	synEntity.annotations = annotDicts
	synEntity = syn.store(synEntity)
	print(synEntity)

'''for index, row in RNA_Seq.iterrows():
	print index, row['id']
	synEntity = syn.get(row['id'], downloadFile = False)
	print synEntity'''
